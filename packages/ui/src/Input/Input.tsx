  ...
  ({ className, variant, size = 'md', leftIcon, rightIcon, ...props }, ref) => {
    ...
    return (
      <Box className="relative w-full">
        {leftIcon && (
          <Box
            className={clsx('absolute left-3 top-0 flex h-full items-center text-gray-400', {
        ...
        {rightIcon && (
          <Box
            className={clsx('absolute right-3 top-0 flex h-full items-center text-gray-400', {
              'text-[var(--color-danger)]': variant === 'error',
              'text-green-500': variant === 'success',
            })}
          >
