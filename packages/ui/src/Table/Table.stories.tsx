...
export const Responsive: Story = {
  render: () => (
    <Box className="w-[22rem] space-y-3">
      <Box
        as="p"
        className="text-sm leading-6 text-muted-foreground"
      >
        Keep the semantic table intact on mobile and let the consumer provide the horizontal
        scroll viewport instead of collapsing columns into a different layout.
      </Box>
      <Box className="overflow-x-auto rounded-lg border border-border bg-background shadow-sm">
        <Table
          className="min-w-[44rem]"
          aria-label="Renewal queue in a constrained mobile viewport"
        >
          <TableCaption>Renewal queue in a constrained mobile viewport.</TableCaption>
          <TableHeader>
            <TableRow>
              <TableHead scope="col" className="w-[140px] whitespace-nowrap">
                Policy
              </TableHead>
          ...
          <TableBody>
            {policyRows.map((policy) => (
              <TableRow key={policy.policyNumber}>
                ...
                <TableCell className="whitespace-nowrap">{policy.holder}</TableCell>
                <TableCell className="whitespace-nowrap">{policy.plan}</TableCell>
                <TableCell className="whitespace-nowrap">{policy.renewal}</TableCell>
  ...
  parameters: {
    ...
    docs: {
      description: {
        story:
          'Exercises the mobile-width composition path where the viewport stays narrow and the consumer-owned shell handles horizontal scrolling.',
      },
    },
  },
};
