...
- Shared field-shell sizing normalization is now landed for selection and date-trigger controls: `Select` and `Combobox` now use `xs | sm | md | lg`, default `md`, alongside the existing `Input`, `DatePicker`, `DateRangePicker`, and `MonthPicker` family; `OtpInput` keeps its shipped public `sm | md | lg` contract; `Pagination`, `Textarea`, `Dialog`, and `Calendar` remain frozen exceptions in this pass.
...
| Textarea        | 4             | B4            | DONE   | Shipped on 2026-03-10 with spec, Storybook, Box-authored multiline field markup, helper/error messaging, and clearable controlled/uncontrolled support; extended on 2026-04-03 with the shared field-variant family so multiline text entry now follows the same `outline                                                                                                                                                                                                                                                                                                                                 | shadow                                                                   | ghost`contract with`outline` as the canonical default                                                            |
