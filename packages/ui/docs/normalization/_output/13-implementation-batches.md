...
- Shared field-shell variant normalization is now landed: `Input`, `Textarea`, `Select`, `Combobox`, `DatePicker`, `DateRangePicker`, `MonthPicker`, `FileUpload`, `OtpInput`, and `RichTextEditor` now align on `outline | shadow | ghost`, with `outline` as the canonical default and legacy `default` retained only as a compatibility alias for `shadow`.
- Shared field-shell sizing normalization is now landed for selection and date-trigger controls: `Select`, `Combobox`, and `Textarea` now use `xs | sm | md | lg`, default `md`, alongside the existing `Input`, `DatePicker`, `DateRangePicker`, and `MonthPicker` family; `OtpInput` keeps its shipped public `sm | md | lg` contract; `Pagination`, `Dialog`, and `Calendar` remain frozen exceptions in this pass.
- Focus normalization is now landed as a bounded follow-up inside `packages/ui`.
- Implemented Wave 1 field-entry coverage: `Input`, `Textarea`, `DatePicker`, `DateRangePicker`, `MonthPicker`, `Select`, `Combobox`, and `OtpInput`, backed by the existing internal focus utility.
- Implemented Wave 2 compact-control coverage: `Button`, `Checkbox`, `RadioGroup`, `Switch`, and `DateRangePicker` preset buttons now use the shared compact-control recipe.
- Implemented Wave 3 dense-surface coverage: `Calendar`, `Tabs`, `Accordion`, `Pagination`, `NavigationMenu`, `Menubar`, `Command`, and dense date-surface follow-up such as the `MonthPicker` year trigger now use the shared dense-surface recipe.
...
| Button          | 4             | B4            | DONE   | Shipped on 2026-03-10 with spec, Storybook, canonical variant/size coverage, loading state, and Box-authored `asChild` composition; refined on 2026-04-23 with explicit `Box as="span"` icon slots for consistent inline-flex alignment                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Card            | 4             | B4            | DONE   | Shipped on 2026-03-10 with spec, Storybook, compound slot exports, and consumer-owned interactive composition; reconciled on 2026-04-03 to expose normalized `outline \| shadow` surface variants with `outline` as the default                                                                                                                                                                                                                                                                                                                                                                           |
...
| Input           | 4             | B4            | DONE   | Shipped on 2026-03-10 with spec, Storybook, normalized input modes, affix slots, clearable behavior, and Box-authored field markup; realigned on 2026-04-03 to the shared field-variant family (`outline \| shadow \| ghost`) with `outline` as the canonical default and legacy `default` preserved as a compatibility alias for `shadow`                                                                                                                                                                                                                                                                |
...
| Pagination      | 4             | B4            | DONE   | Shipped on 2026-03-10 with spec, Storybook, compact page-range handling, first/prev/next/last controls, and optional page-size selector; realigned on 2026-04-03 to the shared navigation-surface variant family with `outline` as the canonical default and legacy `default` preserved as a compatibility alias for `shadow`                                                                                                                                                                                                                                                                             |
...
| Select          | 4             | B4            | DONE   | Shipped on 2026-03-10 as the static single-select primitive; clearable reset support was added on 2026-03-11, 2026-03-27 extended the shared surface with additive Radix-style compound exports while keeping the flat `options` API as the preferred default, and 2026-04-03 added shared field-variant support with `outline` as the canonical default; searchable selection stays on `Combobox`, and multi-select stays out of this contract                                                                                                                                                           |
| Skeleton        | 4             | B4            | DONE   | Shipped on 2026-03-10 with spec, Storybook, Box-authored muted loading surfaces, and decorative-by-default accessibility behavior; refreshed on 2026-04-04 with a clearer shared shimmer treatment backed by semantic-token animation utilities                                                                                                                                                                                                                                                                                                                                                                   |
...
| Switch          | 4             | B4            | DONE   | Shipped on 2026-03-10 with spec, Storybook, Radix-backed toggle semantics, shared size density, error-state support, and Box-authored control + label markup; refreshed on 2026-04-06 so disabled states keep clear track contrast instead of relying on whole-control opacity dimming                                                                                                                                                                                                                                                                                                                     |
...
| Tabs            | 4             | B4            | DONE   | Shipped on 2026-03-10 with spec, Storybook, Radix compound exports, scrollable list support, and Box-authored tab triggers and panels; narrowed on 2026-04-03 by removing the unused `shadow` treatment, then refined again on 2026-04-04 with the public `outline`, `ghost`, and `underline` trigger contract plus an explicit shared overflow track/thumb affordance for long horizontal rails |
| Textarea        | 4             | B4            | DONE   | Shipped on 2026-03-10 with spec, Storybook, Box-authored multiline field markup, helper/error messaging, and clearable controlled/uncontrolled support; extended on 2026-04-03 with the shared field-variant family (`outline \| shadow \| ghost`); and extended on 2026-04-22 with the shared field-shell size family, adding support for `xs \| sm \| md \| lg` (default `md`) while preserving native row-driven height behavior |
...
| DatePicker      | 4             | B5.1          | DONE   | Shipped on 2026-03-12 with spec, Storybook, Input-aligned variant/size shell, optional `formatDate` display override, min/max bounds, clearable value handling, and Box-authored field markup on top of `Calendar` + `Popover`; realigned on 2026-04-03 so the public field shell now defaults to `outline` while legacy `default` remains a compatibility alias for `shadow`                                                                                                                                                                                                                             |
| DropdownMenu    | 4             | B5.1          | DONE   | Shipped on 2026-03-11 with spec, Storybook, compound Radix menu exports, checkbox and submenu support, and Box-authored menu shells; realigned on 2026-04-03 to the shared navigation-surface variant family with root-level and per-item `outline \| shadow \| ghost` support plus legacy `default` alias compatibility                                                                                                                                                                                                                                                                                  |
...
| Combobox        | 4             | B5.2          | DONE   | Shipped on 2026-03-12 with spec, Storybook, cmdk + Popover-backed searchable single-select behavior, loading/error support, and Box-authored wrappers around the documented cmdk DOM boundary; refined on 2026-03-26 with parent-owned search notifications plus a bounded create-on-enter contract for migration-safe async combobox adoption; extended on 2026-03-29 with optional controlled `searchValue` support so parents can keep visible query text in sync during async search migration; realigned on 2026-04-03 with shared field-variant support plus `outline` as the canonical default; and extended on 2026-04-23 with `triggerClassName` support plus hardened internal null-handling for search and option data |
...
| DateRangePicker | 4             | B5.2          | DONE   | Shipped on 2026-03-13 with spec, Storybook, shared preset shortcuts, bounded two-month calendar composition, and Box-authored trigger + preset markup; extended on 2026-03-27 with generic `changeBehavior` support for complete-only parent commits without app-local adapters; and realigned on 2026-04-03 so the public trigger now defaults to `outline` while legacy `default` remains a compatibility alias for `shadow`                                                                                                                                                                            |
| FileUpload      | 4             | B5.2          | DONE   | Shipped on 2026-03-12, refined on 2026-03-17 with drag-active dropzone feedback and generic max-size validation, extended on 2026-03-27 with `displayValue` support for externally supplied filename labels while transport stays local, and extended again on 2026-04-03 with shared field-variant support on the visible dropzone shell                                                                                                                                                                                                                                                                 |
...
| NavigationMenu  | 4             | B5.2          | DONE   | Shipped on 2026-03-13 with spec, Storybook, Radix-backed compound exports, shared indicator + viewport support, and Box-authored wrappers while route trees stay local; realigned on 2026-04-03 to the shared navigation-surface variant family with root-level and trigger/link override support                                                                                                                                                                                                                                                                                                         |
| OtpInput        | 4             | B5.2          | DONE   | Shipped on 2026-03-12 with spec, Storybook, Box-authored segmented input slots, numeric sanitization, whole-code paste handling, and accessible error wiring; realigned on 2026-04-03 to the shared field-variant vocabulary while preserving the segmented-input `sm \| md \| lg` size contract and switching the canonical default surface to `outline`                                                                                                                                                                                                                                                 |
...
| Menubar         | 4             | B5.4          | DONE   | Shipped on 2026-03-12 with spec, Storybook, Radix-backed compound exports, shared shortcut and submenu support, and Box-authored root, trigger, content, and item wrappers; realigned on 2026-04-03 to the shared navigation-surface variant family with root-level plus trigger/item/sub-trigger overrides                                                                                                                                                                                                                                                                                               |
| MonthPicker     | 4             | B5.4          | DONE   | Shipped on 2026-03-13 with spec, Storybook, first-of-month normalization, bounded year navigation, and Box-authored trigger, popover shell, and month-grid controls; realigned on 2026-04-03 so the public trigger now defaults to `outline` while legacy `default` remains a compatibility alias for `shadow`                                                                                                                                                                                                                                                                                            |
...
| RichTextEditor  | 4             | Decision gate | DONE   | Shipped on 2026-03-16 with a bounded Tiptap contract, default sanitization, compact toolbar/link-editor hardening, shared tooltip shortcut coverage for supported actions, and explicit app-local exclusions for uploads, mentions, slash commands, media, viewer-specific rendering, and global shortcut policy; extended on 2026-04-03 with shared field-variant support on the outer editor shell                                                                                                                                                                                                      |
