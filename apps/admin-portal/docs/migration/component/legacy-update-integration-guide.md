...
flowchart TD
    ...
    D --> C
    C -->|KEEP_APP_LOCAL| E[Accept theirs ✅\nAdd to audit.md if missing\nNo packages/ui action]
    C -->|ADOPT_NOW| F[Check if @repo/ui already has it\nIf yes → immediate Batch 1 swap\nIf no → re-classify]
    C -->|EXTEND_EXISTING| G[Add to spec-input.md\nQueue for packages/ui Batch 3\nKeep legacy local file until Batch 3 ships]
    C -->|NEW_SHARED_COMPONENT| H[Add to spec-input.md + backlog.csv\nQueue for packages/ui Batch 4\nKeep legacy local file until Batch 4 ships]
```
...
| ------------------------------------------------------ | -------------------------------- | -------------------------- |
| Domain form (API calls, business logic)                | `KEEP_APP_LOCAL`                 | Keep as-is                 |
| Table column config                                    | `KEEP_APP_LOCAL`                 | Keep as-is                 |
| Page-level layout                                      | `KEEP_APP_LOCAL`                 | Keep as-is                 |
