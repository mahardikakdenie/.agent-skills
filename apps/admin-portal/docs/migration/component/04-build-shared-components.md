...
> **Run count:** Iteratively per batch (Batch 4 -> Batch 5)
...
> **Batch scope:** Batch 1, Batch 2, and Batch 3A require no direct `packages/ui/src` component authoring (those are app-side or foundation-only work). This phase handles Batch 4 (extend existing) and Batch 5 (new components).
...
## Spec and Markup Guardrails

- `ComponentName.spec.md` is mandatory for every Batch 4 and Batch 5 component, including extend-existing work. If a component folder does not contain a current spec, create or update the spec first and stop there.
- Stories are written only after the spec exists; implementation starts only after the stories reflect the spec.
- Inside `packages/ui`, structural wrappers must render through `Box` (`<Box />` or `Box` with the `as` prop) rather than bare `div`, `span`, `section`, `article`, `main`, `aside`, `header`, `footer`, `ul`, `ol`, `li`, or `p`.
- Native elements remain acceptable only when the component itself is that semantic primitive (`input`, `textarea`, `table`, `thead`, `tbody`, `tr`, `th`, `td`, `img`, etc.) or when a Radix/browser primitive requires that underlying element for semantics or behavior.
...
| ------------------------------------------ | :----: | :------: | :-------: |
| `ComponentName.spec.md`                    |   yes   |    yes    |    yes     |
| `ComponentName.tsx`                        |   yes   |    yes    | yes (root) |
| `ComponentName.types.ts`                   |   missing   |    yes    |    yes     |
| `ComponentName.utils.ts`                   |   missing   |    missing    | if needed |
| `ComponentName.hooks.ts`                   |   missing   |    missing    | if needed |
| `parts/ComponentNameXxx.tsx`               |   missing   |    missing    | if needed |
| `ComponentName.stories.tsx`                |   yes   |    yes    |    yes     |
| `ComponentName.stories.parts.tsx`          |   missing   |    missing    | if needed |
| `index.ts`                                 |   yes   |    yes    |    yes     |
| `packages/ui/src/index.ts` (barrel)        |   yes   |    yes    |    yes     |
| `20-foundation-change-log.md` (append)     |   yes   |    yes    |    yes     |
| `21-adapter-mapping.md` (append if needed) |   yes   |    yes    |    yes     |
...
## Batch Prompt (Batch 4 - Extend Existing)
...
1. Create or update `<COMPONENT_NAME>.spec.md` first. If the file is missing, stop at spec creation and do not write stories or code yet.
2. Add stories to `<COMPONENT_NAME>.stories.tsx` for new cases (RED state OK)
4. Implement changes in `<COMPONENT_NAME>.tsx` and any tier-required support files (make Storybook GREEN)
...
- [ ] Storybook story `title` follows `'<Group>/<ComponentName>'` taxonomy ([Section 6.4](./06-component-standards.md#64-storybook-category-taxonomy))
- [ ] Keyboard nav works
...
Document BEFORE writing any stories or code. Every section below is mandatory unless marked _(if applicable)_. The component is not considered started until this file exists in the target folder.
...
| Sub-components | yes - see Section Compound Sub-components \| no | [why] |
...
> **Box rule:** Structural wrappers inside `packages/ui` flow through `Box`; direct native layout tags are reserved for semantic primitives and Radix/browser-required markup.
...
**Story file title:** `'<Group>/<ComponentName>'` - see [Section 6.4 taxonomy](./06-component-standards.md#64-storybook-category-taxonomy)
...
const meta: Meta<typeof ComponentName> = {
  title: '<Group>/ComponentName',      // Section 6.4 taxonomy
...
|---|---|---|---|---|---|---|
| Spinner | 4 | yes | yes | yes | yes | DONE |
| Table | 4 | yes | yes |  |  | IN PROGRESS |
| DataTable | 4 | yes |  |  |  | SPEC REVIEW |
```
