...
> [!IMPORTANT]
> **Preferred Workflow:** If you are running an AI-assisted migration, use **L1â€“L6 batches inside [`migration-batch-prompts.md`](./migration-batch-prompts.md)** instead of this file. The L1â€“L6 prompts there are integrated into the master execution flow, include richer skill suggestions, and reference the correct verification commands.
>
...
- `<APP_PATH>/docs/migration/component/legacy-update-routines.md` â€” Full routine details
- `<APP_PATH>/docs/migration/component/legacy-update-integration-guide.md` â€” Decision tree & quick reference
- **`<APP_PATH>/docs/migration/component/migration-batch-prompts.md`** â€” **Preferred:** L1â€“L6 integrated into master execution file
- `<APP_PATH>/docs/migration/component/00-overview.md` â€” Branch model
- `<APP_PATH>/docs/migration/component/01-app-audit.md` â€” Component classification
- `<APP_PATH>/docs/migration/component/05-app-migration.md` â€” Batch execution
- `<APP_PATH>/docs/migration/component/06-component-standards.md` â€” Normalization standards
- `<APP_PATH>/docs/migration/verification-gate.md` â€” App-specific verification commands (typecheck/lint/build/test)
...
> [!NOTE]
> The batches in this file (Batch 1â€“6) are **legacy update batches**, NOT the same as the main migration batches (Batch 1â€“6 in `03-migration-plan.md`).
>
> | This file                             | Main migration                   | They are different |
> | ------------------------------------- | -------------------------------- | ------------------ |
> | Batch 1 = Subtree Pull & Merge        | Batch 1 = ADOPT_NOW import swaps | âœ… Different scope |
> | Batch 2 = Conflict Resolution         | Batch 2 = ADOPT_WITH_ADAPTER     | âœ… Different scope |
> | Batch 3 = Change Categorization       | Batch 3 = EXTEND_EXISTING        | âœ… Different scope |
> | Batch 4 = Apply Non-Component Changes | Batch 4 = NEW_SHARED_COMPONENT   | âœ… Different scope |
> | Batch 5 = packages/ui Intake          | Batch 5 = Stabilization          | âœ… Different scope |
> | Batch 6 = Verify & Document           | Batch 6 = Cleanup & Deprecation  | âœ… Different scope |
>
> The `migration-batch-prompts.md` file uses the naming **L1â€“L6** (L = Legacy) to avoid this confusion. This file retains the original Batch 1â€“6 naming for backward compatibility.

---

## Key Principle

**`integrate/<APP_NAME>` is 1:1 with the legacy repo** (read-only, never locally modified). Subtree pull to `integrate/<APP_NAME>` will **never conflict**. Conflicts only occur when merging `integrate/<APP_NAME>` â†’ `migrate-app/<APP_NAME>`.
...
**Routine 1 â€” Subtree pull to integrate/<APP_NAME>:**
1. Switch to integrate/<APP_NAME>
2. Run: git subtree pull --prefix=<APP_PATH> <LEGACY_REMOTE> <LEGACY_BRANCH>
   Expected: clean merge â€” no conflicts (integrate/* is read-only, 1:1 with legacy)
3. Push integrate/<APP_NAME>

**Routine 2 â€” Merge to migrate-app/<APP_NAME>:**
...
## Batch 2 â€” Resolve Conflicts on `migrate-app/<APP_NAME>`
...
| Shared infrastructure | packages/config/**, packages/helper/**, tsconfig | Manual merge â€” prefer ours, apply new additions |
| App configuration | package.json, tailwind.config.*, tsconfig.json | Manual merge â€” apply new deps/settings, keep migration overrides |
...
4. After all conflicts resolved:
   git add .
   git commit -m "chore: merge integrate/<APP_NAME> to migrate-app/<APP_NAME> â€” component conflict resolution"
...
**Step 2 â€” Categorize each changed file:**

| Category | Examples | Report |
|---|---|---|
| New component file | New .tsx in src/components/ | List + classify per 06-component-standards.md |
| Existing component modified (non-migrated) | UI update we haven't touched | List â€” already handled by Routine 3 |
| Existing component modified (migrated) | Legacy changed a component we own | ðŸš¨ Flag for review |
| Config/dependency change | package.json, tsconfig, tailwind.config | List each |
| Style/asset change | CSS, public/**, images | List each |

**Step 3 â€” For each NEW component file, classify it:**
...
- Is it purely visual (no API calls, no domain types)?
  YES â†’ Used by 2+ apps? â†’ NEW_SHARED_COMPONENT or EXTEND_EXISTING
  NO  â†’ Classify as KEEP_APP_LOCAL, then run Universal SoC Evaluation:

**Universal SoC Evaluation (required for all KEEP_APP_LOCAL from legacy):**
Per 06-component-standards.md Â§6.2:
...
- If Batch 1.5 candidate = YES:
  â†’ Classification = MIGRATE_AFTER_SPLIT (not KEEP_APP_LOCAL yet)
  â†’ batch = 1.5 in _component-backlog.csv
  â†’ Add to Batch 1.5 queue in update log
- If Batch 1.5 candidate = NO:
  â†’ Classification = KEEP_APP_LOCAL; batch = N/A

**Step 4 â€” Create update log:**
...
## Batch 4 â€” Apply Non-Component Adjustments
...
3. tsconfig / path alias changes:
   - Apply changes
   - Run the exact app typecheck command from `verification-gate.md` Â§1 to verify paths still resolve
...
- **Backward-compatibility contract (mandatory for all changes in this batch):**
  - [ ] No changes to existing component prop APIs for callers â€” new props must be optional; no props removed without migration notice
  - [ ] No removal of existing exports from `@repo/ui` â€” deprecate first if removal is planned
  - [ ] No TypeScript type narrowing â€” widening is OK; narrowing is NOT unless every consumer verified
  - [ ] No hook `queryKey` / `queryFn` signature changes â€” if service track has refactored hooks, legacy updates must not alter those contracts
...
## Batch 5 â€” packages/ui Intake for New Legacy Components
...
3. Keep the legacy local file on migrate-app/<APP_NAME> â€” do NOT delete it yet
   (it will be deleted when packages/ui ships the extension and Batch 3 migration runs)
4. Add note in migration-plan.md: "<ComponentName> â€” waiting for packages/ui Batch 3 extension"
...
1. Document in <APP_PATH>/docs/migration/component/_output/_spec-input.md:
   ...
   - Variants needed: [list]
   - States needed: [default, hover, focus, disabled, loading, error â€” as applicable]
...
4. Add note in migration-plan.md: "<ComponentName> â€” waiting for packages/ui Batch 4 build"
5. Proceed to Batch 6
...
- After Batch 5 queues the intake, run Batch 5A to temporarily re-add the legacy
  component until packages/ui ships it â€” OR wait for packages/ui to ship first
...
## Batch 5A â€” Incremental Migration for Newly Queued Component
...
3. Migrate each usage site (ONE at a time):
   ...
   - Verify the exact app typecheck command from `verification-gate.md` Â§1 still passes
   - Do NOT change unrelated code

4. After ALL usage sites migrated:
   - Run the full verification gate using the exact app commands from `verification-gate.md` Â§1-Â§3
   - Verify app still renders correctly on key routes

5. Update `_output/_migration-log.md`:
   ## <ComponentName> â€” Batch 4 (Incremental â€” legacy update YYYYMMDD)
...
## Batch 6 â€” Verification and Documentation
...
**Step 1 â€” Read verification gate and run commands:**
0. Read <APP_PATH>/docs/migration/verification-gate.md to get the exact verification
   commands for this app (typecheck, lint, build, test, sanity â€” as defined).
   Do NOT guess commands â€” use exactly what is in that file.

Use the exact app typecheck, lint, and build commands from `verification-gate.md` Â§1-Â§3.

If any packages/ui changes were made on feat/ui:
pnpm --filter @repo/ui check-types
pnpm --filter @repo/ui build

**Step 2 â€” Verify migrated component integrity:**
...
**Step 3 â€” If verification fails:**
...
**Step 4 â€” Complete the update log at:**
...
- Current batch position (from migration-plan.md)
- Next steps (e.g., "Resume Batch 4 â€” continue with <ComponentName>")
...
- Verification gate: âœ… all passed
...
## Quick Reference: Scenario â†’ Batch Sequence

| Scenario                                                                     | Batch Sequence                   | Est. Time |
| ---------------------------------------------------------------------------- | -------------------------------- | --------- |
| Clean merge, no new components                                               | 1 â†’ 3 â†’ 4 â†’ 6                    | 30â€“60 min |
| Conflicts, no new components                                                 | 1 â†’ 2 â†’ 3 â†’ 4 â†’ 6                | 1â€“2 hours |
| Clean merge, new KEEP_APP_LOCAL only (SoC = LOW/NONE)                        | 1 â†’ 3 â†’ 4 â†’ 6                    | 30â€“60 min |
| Clean merge, new KEEP_APP_LOCAL with HIGH/MEDIUM SoC (â†’ MIGRATE_AFTER_SPLIT) | 1 â†’ 3 â†’ 4 â†’ 6 + Batch 1.5 queued | 1â€“2 hours |
| Clean merge, new packages/ui candidate                                       | 1 â†’ 3 â†’ 5 â†’ 6                    | 1â€“3 hours |
| Conflicts + new packages/ui candidate                                        | 1 â†’ 2 â†’ 3 â†’ 5 â†’ 6                | 2â€“4 hours |
| Post-cleanup (Batch 5 done) + new candidate now available in @repo/ui        | 1 â†’ 3 â†’ 5 â†’ 5A â†’ 6               | 3â€“5 hours |

---

_Related: [legacy-update-routines.md](./legacy-update-routines.md) Â· [legacy-update-integration-guide.md](./legacy-update-integration-guide.md) Â· [05-app-migration.md](./05-app-migration.md)_
