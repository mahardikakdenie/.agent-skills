...
## Batch 10 - Core Chart Polish, PieChart Parametrization & Formatter Extension - 2026-05-19

- Component focus: `src/components/core/bar-chart-horizontal.tsx`, `src/components/core/line-chart.tsx`, `src/components/core/pie-chart.tsx`, `src/lib/formatter.ts`, `src/app/dashboard/transaction/page.tsx`
- Migration intent: `Polish the three core chart components now living in src/components/core/ after the Batch 10 directory consolidation — improving label rendering in HorizontalBarChart, hardening the YAxis orientation default in LineChart, and parametrizing label-threshold and inner-ring visibility in PieChart. Extend numberSimpleFormatter to handle magnitudes beyond billions. Apply the new PieChart props and add a bottom spacer in the transaction dashboard page.`
- Component-local behavior updates:
  - `apps/admin-portal/src/components/core/bar-chart-horizontal.tsx`: `chartMargin.top` increased from `20` to `36` to give above-bar labels room. A `maxValue` pre-computation (`Math.max(0, ...data.map(...))`) is added before the render return. The `LabelList` inside `<Bar>` is replaced with a custom `content` renderer: it renders `null` for any bar that is not the maximum-value bar or has a non-positive value, and for the max bar it renders a `<text>` SVG element positioned above the bar (`y - 6`, `textAnchor="middle"`, `fill="#374151"`, `fontSize={9}`) using the existing `labelFormatter`. The previous `position="center" fill="white" fontSize={8}` inline approach is removed.
  - `apps/admin-portal/src/components/core/line-chart.tsx`: The `<YAxis orientation>` prop now uses `axis.orientation ?? 'left'` instead of the bare `axis.orientation`, ensuring the recharts `YAxis` always receives an explicit orientation string and does not receive `undefined` when the axis config omits the field.
  - `apps/admin-portal/src/components/core/pie-chart.tsx`: `PieChartCompProps` gains two optional props — `minLabelPercent?: number` (default `0`) and `showInnerPie?: boolean` (default `true`). The inline `renderCustomizedLabel` function is replaced by a `createPercentageLabel(minLabelPercent)` higher-order factory: it returns `null` for any slice whose `percent` is below the threshold, and otherwise renders the same `Box as="text"` percentage label as before. The inner decorative `<Pie>` (outerRadius 50) is now conditionally rendered via `{showInnerPie ? <Pie … /> : null}`, making the donut-only layout opt-in.
  - `apps/admin-portal/src/lib/formatter.ts`: `numberSimpleFormatter` is refactored from a three-tier if/else chain (K, M, B) to a table-driven lookup. The suffix table now covers nine magnitudes: K (1 k), M (1 M), B (1 B), T (1 T), Qa (1 Qa), Qi (1 Qi), Sx (1 Sx), Sp (1 Sp), Oc (1 Oc). `Math.abs(value)` is used for threshold comparison so that negative values abbreviate correctly. The return for unmatched values (< 1 K) remains the raw number. A trailing newline is also added at end of file.
  - `apps/admin-portal/src/app/dashboard/transaction/page.tsx`: `<PieChart>` usage passes `minLabelPercent={0.05} showInnerPie={false}` — hiding the inner ring and suppressing labels for slices smaller than 5 %. A `<Box className="h-5 shrink-0" />` spacer is appended inside `<ContentLoadingWrapper>` to prevent the last grid row from clipping against the container edge.
- Files changed: [`apps/admin-portal/src/components/core/bar-chart-horizontal.tsx`, `apps/admin-portal/src/components/core/line-chart.tsx`, `apps/admin-portal/src/components/core/pie-chart.tsx`, `apps/admin-portal/src/lib/formatter.ts`, `apps/admin-portal/src/app/dashboard/transaction/page.tsx`]
- Shared-ui impact: `No new @repo/ui exports are introduced or changed. All recharts primitives (Bar, LabelList, Line, YAxis, Pie, Cell, Tooltip, ResponsiveContainer) remain the same imports; only their props and surrounding logic are updated.`
- Verification note: `This logging update is based on the current git diff for the five modified files. No new smoke, lint, or build evidence is added in this documentation entry.`
- Tracker impact: `No page migration status changes. This entry records chart polish and utility improvements only; all route migration PASS/FAIL states remain as previously recorded.`
