...
const Select: React.FC<SelectProps> = ({
  ...
  return (
    <Box ref={selectRef} className="relative w-full text-base">
      <Box
        as="button"
        type="button"
        disabled={disabled}
      ...
      >
        ...
        {isPriorityPlaceholder ? (
          <Box as="p" style={placeholderStyle} className={`${placeholderSelectClassName}`}>
            {placeholderSelect}
          </Box>
        ) : (
          <Box as="p" style={placeholderStyle} className={`${placeholderSelectClassName}`}>
      ...
      {isOpen && (
        <Box
          as="ul"
          className={`max-h-36 overflow-y-auto sm:scrollable absolute w-full bg-white border border-gray-300 rounded-md mt-1 z-10 ${dropUp ? 'bottom-full mb-1' : 'top-full mt-1'}`}
        >
          {(allOptions && allOptions.length > 0 ? allOptions : options).map((item, index) => {
            ...
            return (
              <Box
                ...
                onClick={
                  matchingOption ? () => handleSelectOption(matchingOption.value) : undefined
                }
