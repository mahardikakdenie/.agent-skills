  ...
  (
    ...
    const validateForm = () => {
      ...
      if (
        ...
        !!year &&
        minimumDate > '1970-01-01' &&
        minimumDate > `${year}-${month < 10 ? `0${month}` : month}-${date < 10 ? `0${date}` : date}`
      )
        errors.date = `Minimum date is ${minimumDate.split('-').reverse().join(' / ')}`;
    ...
    let maxYear = maximumDate ? new Date(maximumDate).getFullYear() : new Date().getFullYear();

    const years = Array.from({ length: maxYear - currentYear + 1 }, (_, index) => {
    ...
    const generateDates = (month: number, year: number) => {
      ...
      if (minimumDate) {
        ...
        if ((month === minMonth || month === 0) && (year === minYear || year === 0)) {
          startDay = minDay;
        }
      }

      return Array.from({ length: daysInMonth(month, year) - startDay + 1 }, (_, index) => ({
    ...
    return (
      <Box ref={fromUseRef} className="group w-full relative rounded-md shadow-none">
        <Box className="h-full flex flex-col">
          ...
          >
            {label}{' '}
            <Box as="span" className="ml-auto h-5 w-5">
              {CalenderIcon(isDisabled ? primaryDisabled : primary, '30', '30', '0 0 25 24')}
            </Box>
          </Box>
          {errorMessage && (
            <Box as="p" className="text-red-500 text-sm p-1">
              {errorMessage}
            </Box>
          )}
        </Box>
        ...
        {isOpen && (
          <Box
            className={`absolute ${position === 'above' ? 'bottom-full mb-2' : 'top-full mt-2'} left-0 w-full max-w-full bg-white rounded-md shadow-lg p-4 z-10`}
          >
            <Box className="bg-default-100 text-center p-2">
              {title && (
                <Box as="span" className="text-[10px]">
                  {title}
                </Box>
              )}
              <Box className="text-bluedark font-bold text-sm">
                {dateDLabel} {dateMLabel} {dateYLabel}
              </Box>
            </Box>

            <Box className="grid grid-cols-1 gap-3 mt-4">
              <Select
                additionalClassNameSelect="truncate"
                value={year || ''}
                placeholderSelect="Select year"
              ...
              {year !== null && (
                <Select
                  additionalClassNameSelect="truncate"
                  value={month || ''}
                  placeholderSelect="Select month"
            ...
            <Box className="flex items-center justify-center text-center mt-4">
              <Button variant="danger" additionalClassName="mr-2" onClick={handleClear}>
                Clear
              </Button>
