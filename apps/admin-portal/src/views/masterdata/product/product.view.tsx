...
export const MasterdataProductView = () => {
  ...
  return (
    <Box className="mx-auto py-5 px-7">
      ...
      {isMobileView ? (
        ...
        <Box className="overflow-x-auto sm:scrollable text-center flex items-center justify-start h-16 bg-white rounded-md mb-3 shadow">
          {categoryList.map((cs, csIndex) => (
            <Box
              key={`tab-${cs.id}`}
              onClick={() => handleTabChange(cs.id)}
              style={{ width: `calc(100% / ${categoryList.length + 1})` }}
              className={`cursor-pointer h-full flex items-center justify-center ${csIndex === 0 && 'pl-5'} ${csIndex !== categoryList.length - 1 && 'mr-5'} ${tab === cs.id && 'border-b-[3px] border-primary'}`}
            >
              <Box
                as="p"
                className={`text-sm mr-3 ${tab === cs.id && 'font-semibold text-primary'}`}
              >
                {cs.label}
              </Box>
              <Box
                as="p"
                className={`text-center rounded-full bg-red-600 text-white text-xs py-1 ${getPaddingClass(totalData)} ${tab !== cs.id && 'hidden'}`}
              >
      ...
      <Box className="bg-white rounded-md p-5 shadow">
        ...
        <Box className="border rounded-md mt-5">
          <Box className="p-5 bg-primary rounded-t-md flex items-center justify-between">
            <Box as="p" className="text-sm text-white font-medium">
              Product Name
            </Box>
            <Box as="p" className="text-sm text-white font-medium">
              Action
          ...
          <Box>
            {productRows.map((row, index) => (
              <Box
                key={`product-${keyNumber}-${index}`}
                className={`flex items-center justify-between ${index % 2 !== 0 && isNaN(Number(productRows[index].id)) && !row.isEdit && 'bg-gray-100'}`}
              >
                {!isNaN(Number(productRows[index].id)) || row.isEdit ? (
                  <Box className="ml-5 my-3 w-full">
                    <Input
                      value={row.name}
                      onChange={(value) => {
                  ...
                  <Box as="p" className="text-sm p-5 truncate">
                    {row.name}
                  </Box>
                )}
                <Box className="flex items-center ml-3 my-3 mr-5">
                  {!isNaN(Number(productRows[index].id)) || row.isEdit ? (
                    <Box
                      className="clickable"
                      onClick={() =>
                        createOrUpdateProduct(index, productRows[index].id, productRows[index].name)
                      }
                    >
                      {ChecklistIcon()}
                    </Box>
                    ...
                    >
                      {EditIcon('#000', '16', '16', '0 0 20 20')}
                    </Box>
                  )}
                  <Box
                    onClick={() => deleteProduct(productRows[index].id)}
