...
export const DashboardPolicy = () => {
  ...
  useEffect(() => {
    const fetchDataPolicy = async () => {
      try {
        ...
        const response: any = await policyService.getPolicyStatistics(params as any);
  ...
  const groupedData: { name: string; value: number }[] = Array.isArray(policiesStatisticData)
