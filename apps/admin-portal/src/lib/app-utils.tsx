    ...
    {breadcrumbs &&
      breadcrumbs.map((bc, index) => (
        <Box key={index} className="flex items-center justify-between">
          ...
          <Box as="p" className="text-xs text-primary">
            {capitalizeString(bc)}
          </Box>
        </Box>
      ))}
...
export const forLabelString = (docData: any) => {
  ...
  if (!!docData && !!docData.label && typeof docData.label === 'object' && !!docData.label.en) {
    ...
    !!docData &&
    !!docData.label_multilanguage &&
    typeof docData.label_multilanguage === 'object' &&
    !!docData.label_multilanguage.en
  ) {
  ...
  if (!!docData && !!docData.insured_type)
    return filename2Str(finalLabelString, docData.insured_type);
