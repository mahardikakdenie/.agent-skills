...
const Input: React.FC<InputProps> = ({value, type = "text", onChange, onChangeFile, accept = ".csv", onEnter, disabled = false, withBorder = true, isCurrency = false, currency, max, min, placeholder, errorMessage, icon, onClear}) => {
            ...
            <div className="flex items-center">
                ...
                {disabled ? null : inputValue ? (
                    <div className="flex items-center justify-center absolute right-2">
                        ...
                        {type === "password" && !isShow && (<div onClick={() => togglePassword("text")} className="ml-1.5 cursor-pointer">{SeeIcon(primary)}</div>)}
                        {isShow && (<div onClick={() => togglePassword("password")} className="ml-1.5 cursor-pointer">{UnseeIcon(primary, "21", "20", "0 3 27 20")}</div>)}
                    </div>
                ) : icon ? (
