...
const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent }: any) => {
  ...
  return (
    <text x={x} y={y} fill="white" fontSize="12px" fontWeight="bold" textAnchor="middle" dominantBaseline="central">{`${(percent * 100).toFixed(0)}%`}</text>
  );
};
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-white text-gray-700 text-xs p-4 border rounded-md shadow-md">
        ...
        <p className="font-semibold mt-2">{value} ({percentage}%)</p>
      </div>
    );
  }
  return null;
  ...
  return (
    <div className="flex justify-center items-center w-full h-full">
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
        <Pie data={filteredData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={50} fill="#8884d8" startAngle={90} endAngle={-270} />
          <Pie data={filteredData} cx="50%" cy="50%" labelLine={false} label={renderCustomizedLabel} outerRadius={130} innerRadius={80} dataKey="value" startAngle={90} endAngle={-270} >
