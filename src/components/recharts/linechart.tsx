import { formatMoney } from "@/helpers/app.helper";
import {
  ResponsiveContainer,
  Legend,
  Tooltip,
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white text-gray-700 text-xs p-4 border rounded-md shadow-md">
        ...
        <p className="text-xs">
          Total Claims: <strong>{formatMoney(payload.find((p: any) => p.dataKey === "count")?.value)}</strong>
        </p>
        <p className="text-xs">
          Total Claim Amount: <strong>{formatMoney(payload.find((p: any) => p.dataKey === "total_claim_amount")?.value)}</strong>
        </p>
      </div>
    );
  }
