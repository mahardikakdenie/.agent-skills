...
export const ScreenProvider: React.FC<ScreenProviderProps> = ({ children }) => {
    ...
    return (
        <ScreenContext.Provider value={{ isMobileView, isLoading, isMenuOpen, setLoading, setMenu }}>
            {isLoading && <Loader />}
            {!isNetworkActive && (
                <Modal widthClassName="lg:w-[500px]" heightClassName="h-fit" isOpen={isModalNetworkOpen} onClose={setIsNetworkActiveToTrue}>
                    <div className="flex flex-col items-center justify-center py-5">
                        <div className="flex items-center justify-center mb-3">
                            {WifiOffIcon(undefined, "70", "70", "0 0 24 24")}
                        </div>
                        <h1 className="font-bold text-lg text-center mb-2">Oops! Your internet connection is not active or the server is under maintenance.</h1>
                        <p className="text-center text-xs">Please check your internet and try again later!</p>
                        <p className="mb-3 text-center text-xs">Don&apos;t reload this page if you are doing transaction, just close this pop up and trigger again.</p>
