...
export const LayoutView = ({ children }: Readonly<{ children: React.ReactNode }>) => {
  ...
  const [validations, setValidations] = useState({ minLength: false, hasUpperCase: false, hasLowerCase: false, hasNumber: false, hasSpecialChar: false });
  ...
  const { user, menuList, submenuList, isAuthenticated, isForbidden, login, logout } = useAuth();
  ...
  const changePassword = async () => {
    try {
      await authService.put(ApiURL.v1ChangePassword(user.sub), { newPassword, oldPassword });
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {requirements.map((req, index) => (
                <div key={index} className="flex items-center gap-2 text-xs">
                  <div>{req.isValid ? ChecklistIcon("#00AB4F", "16", "16") : XIcon(primaryRed, "16", "16", "0 0 23 23")}</div>
                  {req.text}
                </div>
            ))}
          </div>
  ...
  return (
      <div>
                ...
                <div className="flex items-center justify-center mb-3 -ml-5">
                  <div className={`${!!process.env.NEXT_PUBLIC_LOGO && "py-5 px-2"}`}>
                    ...
                    {process.env.NEXT_PUBLIC_MODE === "whitelable" ? (
                        <OptimizeImage priority width={250} height={100} alt="whitelable-logo-login" src={whitelableLogo} />
                    ) : (
                        <OptimizeImage priority width={logoWidth} height={logoHeight} alt="logo-login" src={logo} />
                    )}
                  </div>
                </div>
                <div className="w-3/4">
                  <div className="mb-5">
                    <Input value={email} onChange={(value) => setEmail(value.toString())} onEnter={doLogin} placeholder="Email" withBorder={true} />
                  </div>
                  <div>
                    <Input type="password" value={password} onChange={(value) => setPassword(value.toString())} onEnter={doLogin} placeholder="Password" withBorder={true} />
                  </div>
                </div>
                <Button additionalClassName="my-5" onClick={doLogin}>Login</Button>
              </div>
            </Modal>
        )}

        <nav className={`px-6 navbar-top bg-primary sticky top-0 left-0 ${isMenuOpen && !isMobileView ? "sm:w-[calc(100%-20%)] sm:ml-[20%]" : "w-full"} ${!user && "w-full"} transition-all duration-300 z-50`} ref={navbarRef}>
          <div className="w-full outline-none">
            <div className="py-2">
              <div className="items-center flex">
                ...
                <div className="relative flex items-center gap-1 cursor-pointer" onClick={() => setShowDropdown(!showDropdown)} ref={dropdownRef}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center justify-between mr-3">
                      <p className="text-sm text-white">Hi, {user?.name || "Anonymous"}</p>
                      <div className={`transition-transform duration-300 ${showDropdown ? "rotate-180" : "rotate-0"}`}>
                    ...
                    <div className="rounded-full h-8 w-8 bg-primary-light-foreground flex items-center justify-center">
                      <p className="text-primary font-semibold">{user?.name?.charAt(0).toUpperCase() || "A"}</p>
                    </div>
                  </div>
                  ...
                  {showDropdown && (
                      <div className="w-52 absolute top-full right-0 mt-2 bg-white shadow-lg rounded-md py-5 px-7">
                        <div className="flex flex-col items-center justify-center">
                          <Button variant="warning" onClick={() => setIsModalChangePassword(true)} additionalClassName="w-full">Change Password</Button>
                          <Button variant="danger" onClick={doLogout} additionalClassName="w-full mt-3">Logout</Button>
                        </div>
                      </div>
                  )}
                </div>
        ...
        <main
            className={`overflow-y-auto sm:scrollable bg-[#F8F8F8] ${isMenuOpen && !isMobileView ? "sm:w-[calc(100%-20%)] sm:ml-[20%]" : "w-full"} transition-all duration-300`}
            style={{ backgroundImage: backgroundImageApp, backgroundRepeat: "no-repeat", backgroundSize: "30%", backgroundPosition: "bottom right", minHeight: "100vh" }}
        >
          {isForbidden ? (
        ...
        {isModalChangePassword && (
            <Modal widthClassName={`w-[500px] ${isMobileView ? 'px-5' : 'px-10'} px-5`} heightClassName="h-fit" isOpen={isModalChangePassword} onClose={() => setIsModalChangePassword(false)}>
              ...
              <div className="h-fit max-h-[calc(70vh-50px)] overflow-y-auto sm:scrollable mb-5">
                <div className="text-sm mb-5">Choose a strong password and don&#39;t reuse it for other accounts.</div>
                <div className="w-full">
                  <div className="mb-5">
                    <Input type="password" value={oldPassword} onChange={(value) => setOldPassword(value.toString())} placeholder="Old Password" withBorder={true} />
                  </div>
                  <div className="mb-5">
                    <Input type="password" value={newPassword} onChange={(value) => handleChangeNewPassword(value.toString())} placeholder="New Password" withBorder={true} />
                    {showPasswordRequirements ? renderPasswordRequirements() : null}
                  </div>
                  <div className="mb-5">
                    <Input type="password" value={confirmNewPassword} onChange={(value) => setConfirmNewPassword(value.toString())} placeholder="Confirm New Password" withBorder={true} errorMessage={newPassword !== confirmNewPassword ? "Passwords do not match. Please re-enter your confirmation password." : undefined} />
                  </div>
                </div>
                <div className="text-sm mb-5">Once your password has been changed, please log back in with the new password on all your devices.</div>
                <div className="flex justify-center items-center">
                  <Button variant="warning" onClick={changePassword}>Change Password</Button>
                </div>
              </div>
            </Modal>
        )}
        ...
        {isMobileView ? (
            <Modal widthClassName="w-[500px]" heightClassName="h-fit" isOpen={isMenuOpen} onClose={() => setMenu(false)}>
              <div className="flex justify-center items-center mb-3 bg-white rounded-t-md">
                <div className={`${!!process.env.NEXT_PUBLIC_LOGO && "py-5 px-2"}`}>
                  {/*TODO: change for customization in env*/}
                  {process.env.NEXT_PUBLIC_MODE === "whitelable" ? (
                      <OptimizeImage priority width={250} height={100} alt="whitelable-logo-mobile" src={whitelableLogo} />
                  ) : (
                      <OptimizeImage priority width={logoWidth} height={logoHeight} alt="logo-mobile" src={logo} />
                  )}
                </div>
              </div>
              <div className="h-fit max-h-[calc(70vh-50px)] overflow-y-auto sm:scrollable mb-5">
                {AppMenu.menu.map((menuMobile, menuMobileIndex) => menuList.includes(menuMobile.name) && (
            ...
            <div className={`fixed top-0 left-0 h-full px-5 py-2 hidden sm:inline bg-white transition-transform duration-300 ease-out ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`} style={{ width: "20%" }}>
              <div className="h-full flex flex-col">
                <div className="flex justify-center items-center">
                  <div className={`${!!process.env.NEXT_PUBLIC_LOGO && "py-5 px-2 mb-3"}`}>
                    {/*TODO: change for customization in env*/}
                    {process.env.NEXT_PUBLIC_MODE === "whitelable" ? (
                        <OptimizeImage priority width={250} height={100} alt="whitelable-logo" src={whitelableLogo} />
                    ) : (
                        <OptimizeImage priority width={logoWidth} height={logoHeight} alt="logo" src={logo} />
                    )}
                  </div>
                </div>
                <div className="overflow-y-auto sm:scrollable flex-1">
                  {AppMenu.menu.map((menu, menuIndex) => menuList.includes(menu.name) && (
