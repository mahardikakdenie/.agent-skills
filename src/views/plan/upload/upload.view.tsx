...
import {masterdataNotV1Service, masterdataService} from "@/services/api.service";
import {capitalizeString, capitalizeStringWithChar, getBreadcrumbs, toastNotification, toCamelCase} from "@/helpers/app.helper";
...
export const UploadPlanView = () => {
        ...
        const fetchPlanDetails = async () => {
            try {
                ...
                if (responsePlanDetails) {
                    const key = toCamelCase(`${responsePlanDetails.data.data[0]?.products?.categories?.name || ""} ${doc?.toString() || ""}`);
    ...
    const uploadToServer = async () => {
        try {
            ...
            let response: any;
            if (!id || !doc) return;
            if (doc.toString() === "benefits") response = await masterdataService.post(`${ApiURL.planBenefit}/bulk-create/${id.toString()}`, csvData);
            else if (doc.toString() === "packages") response = await masterdataNotV1Service.post(`${ApiURL.packages}/${category}/bulk-create/${id.toString()}`, csvData);
            else response = await masterdataService.post(`${ApiURL.plans}/bulk-create/${id.toString()}/${selectedDetailOption}`, csvData);
    ...
    const filenameTemplate = (str1: string, str2: string) => {
        if (str1 && str2) return `Download ${capitalizeStringWithChar(str1, "-").toLowerCase()} ${capitalizeStringWithChar(str2, "-").toLowerCase()}`;
        ...
        <div className="mx-auto">
            <div className="overflow-x-auto sm:scrollable bg-white flex items-center justify-between mb-5 py-5 px-7">
                <div>
                    ...
                    <p className="font-bold text-lg">Upload {capitalizeString(doc ? doc.toString() : '')}</p>
                </div>
                <div onClick={goToPlanDetailPage} className="flex items-center justify-between cursor-pointer">
            ...
            <div className="pb-5 px-7 mb-5">
                <div className="p-5 bg-white rounded-md shadow">
                    ...
                    {!!templateFileUrl && doc&& (
                        <div className="mt-2">
                            <p className="text-xs">{filenameTemplate(plan?.products?.categories?.name, doc?.toString())} template <span className="text-primary font-medium clickable" onClick={downloadTemplate}>here</span>.</p>
                        </div>
                    )}
                    <div className="mt-4">
                    ...
                    <div className="flex items-center text-center mt-4">
                        <Button variant="warning" additionalClassName="mr-2" disabled={doc && doc.toString() === "details" ? (!fileToUpload || !selectedDetailOption) : !fileToUpload} onClick={handlePreview}><span className={`mx-3.5 ${doc && doc.toString() === "details" ? ((!fileToUpload || !selectedDetailOption) && "text-white") : (!fileToUpload && "text-white")}`}>Preview</span></Button>
                        <Button disabled={!isReadyToUpload} onClick={uploadToServer}><span className="mx-3">Upload</span></Button>
                    </div>
                </div>
            </div>
            {isReadyToUpload && (
                        ...
                        <div className="overflow-x-auto sm:scrollable">
                            <table className="min-w-full divide-y divide-gray-200">
                                ...
                                <tr>
                                    ...
                                    {csvData.length > 0 && Object.keys(csvData[0]).map((d, idx) => (
                                        <th key={`header-${idx}`} className="px-6 py-6 whitespace-nowrap text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{d}</th>
                                    ))}
                                </tr>
                                </thead>
                                <tbody className="bg-white divide-y divide-gray-200">
                                {csvData.length > 0 && csvData.map((d, idx) => (
                                    <tr key={`data-${idx}`}>
                                        <td className="w-1/12 px-6 py-3 whitespace-nowrap text-sm text-gray-500">{idx + 1}</td>
                                        {Object.values(d).map((s: any, sIdx) => (
                                            <td key={`subdata-${sIdx}`} className="w-1/12 px-6 py-3 whitespace-nowrap text-sm text-gray-500">{s}</td>
                                        ))}
                                    </tr>
                                ))}
                                </tbody>
