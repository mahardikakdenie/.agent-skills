...
export default function EditSanctionPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        if (params.id) {
            sanctionService.get(ApiURL.v1BlacklistDetails(params.id as string))
                .then((response: any) => {
    ...
    const handleSave = async (formData: any) => {
        ...
        try {
            ...
            // Update the sanction
            const response: any = await sanctionService.put(ApiURL.v1BlacklistUpdateDetails(params.id), payload);
            const { data } = response;
