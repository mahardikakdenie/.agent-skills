...
const AssignPlan = ({
  ...
  const handleAssign = async (planId: string) => {
    try {
      await productCatalogService.assignPlans(planId, id, channelName);
  ...
  const handleUnassign = async (planId: string) => {
    const channelPlanId = getChannelPlanId(planId);
    if (!channelPlanId) return;

    try {
      await productCatalogService.unAssignPlans(planId, id);
      // Refresh assigned plans
      const response = await productCatalogService.getChannelPlansByChannel(id);
      if (response) {
        setAssignedPlans(response);
      ...
      console.error("Failed to unassign plan:", error);
    }
  };
