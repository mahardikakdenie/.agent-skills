...
// import { DashboardTransaction } from "@/views/dashboard/transaction/transaction.view";
//
...
export default function DashboardTransaction() {
  const [transactionStatisticData, setTransactionStatisticData] = useState<any>(null);
    ...
    const fetchDataTransaction = async () => {
      try {
        const params = {
          ...
          from: from ? from : thirtyDaysLater.toISOString().split("T")[0],
          to: to ? to : today.toISOString().split("T")[0]
        };
        const response: any = await transactionService.get(ApiURL.v1TransactionsStatisticData, { params });
