...
// import { EndorsementDetailView } from "@/views/policy/endorsement/detail/detail.view";
//
// export default function EndorsementDetailPage() {
//   return <EndorsementDetailView />;
// }
...
export default function DetailEndorsement() {
  ...
  useEffect(() => {
    const fetchEndorsementData = async (id: string) => {
      try {
        const endorsementDetailResponse: any = await policyService.get(ApiURL.v1EndorsementDetail(id));
        setEndorsement(endorsementDetailResponse.data);
      } catch (error) {
  ...
  const rejectedModal = async (id: string) => {
    try {
      await policyService.put(ApiURL.v1EndorsementUpdateStatus(id), { status: "Rejected", note: notes });
  ...
  const confirmModal = async (id: string) => {
    try {
      await policyService.put(ApiURL.v1EndorsementUpdateStatus(id), { status: "Approved", note: "", });
      window.location.reload();
    } catch (error) {
      alert(error);
    }
  ...
  const confirmModalEdsb = async (id: string) => {
    try {
      await policyService.put(ApiURL.v1EndorsementUpdtaeStatusBulking(id), { status: "Approved", note: "", });
      window.location.reload();
    } catch (error: any) {
  ...
  const rejectedModalEdsb = async (id: string) => {
    try {
      await policyService.put(ApiURL.v1EndorsementUpdtaeStatusBulking(id), { status: "Rejected", note: notes, });
      window.location.reload();
    } catch (error: any) {
  ...
  return (
      <div className="flex flex-col w-full">
        <div className="bg-white md:px-6 p-4 flex items-center">
          ...
          <a href={AppURL.endorsementList} className="font-semibold ml-auto items-center flex gap-1 text-red-700 text-sm cursor-pointer">
                  ...
                  <div className="ml-auto flex gap-2 items-center">
                    {endorsement?.status_description === "Uploaded by partner" && (
                        <Button
                            ...
                            onClick={() => router.push(`${AppURL.endorsementDetail}/${endorsement.id}/upload`)}
                        >
