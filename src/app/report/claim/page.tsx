...
export default function ReportClaimPage() {
    ...
    if (date?.from && date?.to) {
      ...
      if (hasAccess) {
        const params = {
          ...
          limit: rowsPerPage,
          output: "Data",
          date_from: date?.from ? date?.from?.toISOString().split("T")[0] : undefined,
          date_to: date?.to ? date?.to?.toISOString().split("T")[0] : undefined
        };
        claimService.get(ApiURL.v1ClaimsExport, { params })
          .then((res: any) => {
            if (res?.data?.data && res?.data?.data?.length > 0) {
              // Extract headers from the first item
              const headerKeys = Object.keys(res.data?.data?.[0]);
  ...
  const handleDownloadReport = async () => {
    try {
      ...
      const response: any = await claimService.get(ApiURL.v1ClaimsExport, { params });
