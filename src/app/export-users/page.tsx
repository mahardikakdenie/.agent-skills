...
const ExportUsersPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchProductList = async (
      ...
      try {
        const productResponse = await productService.get100Products(
          currentPage
        );
    ...
    fetchChannelList().then(() =>
      fetchProductList().then(() => fetchPlanList().then())
    );
  ...
  const fetchData = async (
    ...
    selectedPlan: string[],
    selectedFrequentBuyers: string[],
    selectedBirthdayMonth: string[],
    isFetchAllData: boolean = false
  ) => {
    try {
      setLoading(true);
  ...
  const fetchAllDataToDownload = async (
    selectedChannel: string[],
    selectedProduct: string[],
    selectedPlan: string[],
    selectedFrequentBuyers: string[],
    ...
    try {
      const response: any = await transactionService.getCustomersCampaign(
        currentPage,
        1000,
        selectedChannel,
        selectedProduct,
  ...
  const processingFilteredUser = () => {
    const selectedChannel: string[] = filteredUsers
      .filter((f) => f.key_id === "channel_id")
      .map((f) => f.value);
    const selectedProduct: string[] = filteredUsers
      .filter((f) => f.key_id === "product_id")
      .map((f) => f.value);
    const selectedPlan: string[] = filteredUsers
      .filter((f) => f.key_id === "plan_id")
      .map((f) => f.value);
    const selectedFrequentBuyers: string[] = filteredUsers
      .filter((f) => f.key_id === "frequent_buyers")
      .map((f) => f.value);
    const selectedBirthdayMonth: string[] = filteredUsers
