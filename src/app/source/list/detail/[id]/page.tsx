...
export default function EditSourcePage() {
  ...
  useEffect(() => {
    if (id) {
      sanctionService
        .get(ApiURL.v1SourcesDetails(id as string))
        .then((response: any) => {
  ...
  const handleSave = async (formData: any) => {
    ...
    if (!urlPattern.test(formData.source_url)) {
      setErrorMessage(
        'URL must start with "https://" and be a valid URL with at least one dot.'
      );
      setShowAlert(true);
      return;
    }
    ...
    const payload = {
      ...
      insurance_name:
        source.source_type === "insurance" ? source.insurance_name : null,
    ...
    try {
      ...
      const response: any = await sanctionService.put(
        ApiURL.v1SourcesUpdateDetails(id),
        payload
      );
