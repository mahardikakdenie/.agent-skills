...
export default function EditSourcePage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        if (params.id) {
            sanctionService.get(ApiURL.v1SourcesDetails(params.id as string))
                .then((response: any) => {
    ...
    const handleSave = async (formData: any) => {
        ...
        try {
            // Update the source
            const response: any = await sanctionService.put(ApiURL.v1SourcesUpdateDetails(params.id), payload);
