...
const EditPartnerComPage = () => {
  const { id } = useParams();
  const { getChannelFees, channelFees, updateChannelFee } = useBrokerFee();
  const {
    insurances,
    products,
    plans,
  ...
  const watchChannel = useWatch({ control, name: "channel" });
  ...
  const getChannels = async () => {
    try {
      ...
      const channelResponse = await channelService.getChannels(0, 10000);
...
const EditBrokerFeePageWithSidebar = (params: any) =>
  WithSidebar(EditPartnerComPage)(params);
