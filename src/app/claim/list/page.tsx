...
// import {ClaimListView} from "@/views/claim/list/list.view";
//
...
export default function ClaimsPage() {
    ...
    const [searchSlaStatus, setSearchSlaStatus] = useState("All");
            ...
            try {
                const params = {
                    ...
                    status: tab === "All" ? [
                        ...
                        "Paid",
                        "Closed",
                    ] : tab !== "Draft" ? [tab] : undefined,
                    keyword: searchData || undefined,
                    sla_status: searchSlaStatus === "All" ? undefined : searchSlaStatus,
                ...
                const res: any = await claimService.get(ApiURL.v1Claims, { params });
    ...
    const selectChannel = async (id: string) => {
        await claimService.get(ApiURL.v1ClaimChannelFormsAll(id)).then((res) => {
            setDataDocument(res?.data?.data);
        });
    };

    const selectCategory = async (id: string, dataId: string) => {
        await claimService.get(ApiURL.v1ClaimCategoryFormsAll(id)).then((res: any) => {
            const label = filteredClaims?.filter((f: any) => f?.id === dataId)?.[0]
                ?.claim_config;

            const updatedDataDocument = res?.data?.data
                .filter(
                    (doc: any) =>
    ...
    const updateStatus = async (
        ...
        await claimService
            .put(ApiURL.v1ClaimUpdateStatus(claimId), body)
            .then(() => {
    ...
    useEffect(() => {
        const fetchClaimsStatus = async () => {
            try {
                const response: any = await claimService.get(ApiURL.v1ClaimConfigurations);
    ...
    return (
        <div className="flex flex-col w-full p-4 md:p-6 ">
            <div className="flex flex-wrap justify-end gap-4 pb-4 items-center">
                ...
                <Button onClick={() => router.push(`${path}/import`)} className="bg-[#016DA1] text-white hover:bg-[#0482C2] rounded-full">
                    <Upload className="w-5 h-5 mr-1" /> Import
                </Button>
                {/* New button to redirect to the new import page with preview */}
                <Button onClick={() => router.push(`${path}/import-with-preview`)} className="bg-[#016DA1] text-white hover:bg-[#0482C2] rounded-full">
