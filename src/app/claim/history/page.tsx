...
import { ClaimHistorySummary, ClaimHistoryDetail } from "@/services/claim.service";
...
export default function ClaimHistoryPage() {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      ...
      const hasAccess = permissionList.includes(`${capitalizeString(headerPage.breadcrumbsArray[0])}.${headerPage.pageName}.Read`);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await claimService.get(ApiURL.v1ClaimListLimit, { params });

        // Validate the response
        const mappedResponse = mapResponse(res.data.data);
