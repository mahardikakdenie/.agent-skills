...
export default function EditPromotionPage() {
  ...
  useEffect(() => {
    if (id) {
      promotionService
        .get(ApiURL.v1CampaignDetail(id), { params: { id } })
        .then((response: any) => {
          const res = response.data;
          const promotionData: PromotionDetails = res.data[0];
          const formattedStartDate = promotionData.start_date
            ? formatDate(promotionData.start_date)
            : "";
          const formattedEndDate = promotionData.end_date
          ...
          const existingChannelIds = new Set(
            promotionData.embedded_discount_channels.map(
              (channel) => channel.channel_id
            )
          );
          setGlobalSelectedChannels(existingChannelIds);

          const existingInsuranceIds = new Set(
            promotionData.embedded_discount_insurances.map(
              (ins) => ins.insurance_id
            )
          );
          ...
          const existingProductIds = new Set(
            promotionData.embedded_discount_products.map(
          ...
          const existingPlanIds = new Set(
            promotionData.embedded_discount_plans.map((plans) => plans.plan_id)
          );
  ...
  useEffect(() => {
    const existingInsuranceIds = new Set(
      promotion.embedded_discount_insurances.map((ins) => ins.insurance_id)
    );
  ...
  const fetchPlansByProducts = async (
    ...
    try {
      const responses: any = await productService.get(ApiURL.v1Plans, {
        params: { productIds, pageSize: limit, page },
      });
  ...
  const fetchPlansByProductsInitial = async (
    productIds: string[],
    page: number,
    limit: number
  ) => {
  ...
  const fetchProductsByInsurances = async (
    ...
    try {
      const allProducts: any = await productService.get(ApiURL.v1Products, {
        params: { insuranceIds, pageSize: limit, page },
      });
  ...
  const fetchProductsByInsurancesInitial = async (
    insuranceIds: string[],
    page: number,
    limit: number
  ) => {
  ...
  const handleAddVoucher = async (code: string, usageLimit: number) => {
    ...
    const voucherVerify: any = await promotionService.get(
  ...
  const handleSearch = async (query: string) => {
    try {
      const res: any = await productService.get(ApiURL.v1Plans, {
        params: {
                ...
                <div className="border rounded bg-white overflow-y-auto flex-grow mr-2 h-32">
                  <div className="flex flex-col p-2">
                    {Array.from(globalSelectedInsuranceIds).length > 0 ? (
                      Array.from(globalSelectedInsuranceIds).map(
                        (insId, index) => {
                          const insuranceDetail =
                            insurancesInitial?.data.find(
                              (c: any) => c.id === insId
                ...
                <div className="border rounded bg-white overflow-y-auto flex-grow mr-2 h-32">
                  <div className="flex flex-col p-2">
                    {Array.from(globalSelectedProdIds).length > 0 ? (
                      Array.from(globalSelectedProdIds).map(
                        (product, index) => {
                          const productDetail =
                            productsInitial?.data.find(
                              (p: any) => p.id === product
                  ...
                  <div className="flex flex-col p-2">
                    {Array.from(globalSelectedPlanIds).length > 0 ? (
                      Array.from(globalSelectedPlanIds).map((plan, index) => {
                        const planDetail =
                          plansInitial?.data.find((p: any) => p.id === plan) ||
