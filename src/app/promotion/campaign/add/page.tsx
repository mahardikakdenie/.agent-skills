...
export default function CreatePromotionPage() {
  ...
  const fetchPlansByProducts = async (productIds: string[], page: number, limit: number) => {
    try {
      const responses: any = await productService.get(ApiURL.v1Plans, { params: { productIds, page, pageSize: limit } });
  ...
  const handleSearch = async (query: string) => {
    try {
      const response: any = await productService.get(ApiURL.v1Plans, { params: { productIds: Array.from(globalSelectedProdIds), page: 1, pageSize: 10, planName: query } });
  ...
  const fetchProductsByInsurances = async (
    ...
    try {
      const allProducts: any = await productService.get(ApiURL.v1Products, { params: { insuranceIds, page, pageSize: limit } });
  ...
  const handleAddChannel = () => {
    const selectedChannelIds: Set<string> = new Set(
      promotion.embedded_discount_channels.map((channel: any) => channel.channel_id)
    );
  ...
  const handleAddIns = () => {
    const selectedInsIds: Set<string> = new Set(
      promotion.embedded_discount_insurances.map(
        (insurances: any) => insurances.insurance_id
      )
    );
  ...
  const handleAddProduct = () => {
    const selectedProdIds: Set<string> = new Set(
      promotion.embedded_discount_products.map(
        (products: any) => products.product_id
      )
    );
  ...
  const handleSave = async (formData: any) => {
    ...
    const payload = {
      ...
      products: promotion.embedded_discount_products.map((product: any) => ({
        product_id: product.product_id,
        name: product.product_name
      })),
      insurances: promotion.embedded_discount_insurances.map((insurance: any) => ({
        insurance_id: insurance.insurance_id,
        name: insurance.insurance_name
      })),
      plans: promotion.embedded_discount_plans.map((plan: any) => ({
        plan_id: plan.plan_id,
        name: plan.name
      })),
      channels: promotion.embedded_discount_channels.map((channel: any) => ({
    ...
    try {
      ...
      for (const element of payload.vouchers) {
        const voucherVerify: any = await promotionService.get(ApiURL.v1VoucherCode(element.code));
        const { data } = voucherVerify?.data;
      ...
      if (voucherExists) {
        ...
        const response: any = await promotionService.post(ApiURL.v1Campaign, payload);
        const { data } = response;
  ...
  const handleSelectChannel = (selectedChannels: any[]) => {
  ...
  const handleSelectProduct = (selectedProducts: any[]) => {
    ...
    setPromotion((prevState: any) => {
      const updatedPlans = prevState.embedded_discount_plans.filter((plan: any) =>
  ...
  const handleAddPlan = () => {
    const selectedPlanIds: Set<string> = new Set(
      promotion.embedded_discount_plans.map((plans: any) => plans.plan_id)
    );
  ...
  const handleRemoveProduct = (index: number) => {
    setPromotion((prevState: any) => {
      ...
      const updatedPlans = prevState.embedded_discount_plans.filter((plan: any) =>
        updatedProducts.some((product: any) => product.product_id === plan.plan_id)
      );

      const updatedSelectedPlanIds = new Set(selectedPlanIds);
      prevState.embedded_discount_plans.forEach((plan: any) => {
        if (!updatedPlans.some((p: any) => p.plan_id === plan.plan_id)) {
      ...
      setGlobalSelectedProdIds((prevSelected) => {
        ...
        updatedProducts.forEach((prod: any) => newSelected.add(prod.product_id));
  ...
  const handleRemoveInsurance = (insuranceId: string) => {
    setPromotion((prevState: any) => {
      const updatedInsurances = prevState.embedded_discount_insurances.filter(
        (insurance: any) => insurance.insurance_id !== insuranceId
      );
      ...
      <form onSubmit={handleSubmit(handleSave)} className="w-full">
        <div className="bg-white md:px-6 p-4 flex items-center">
          ...
          <div className="flex space-x-4 ml-auto">
            <div
              onClick={() => router.push(AppURL.promotionCampaign)}
        ...
        <PlanSelectionModal
          ...
          preSelectedPlanIds={
            new Set(promotion.embedded_discount_plans.map((p: any) => p.plan_id))
          }
          selectedProductIds={
            new Set(
              promotion.embedded_discount_products.map((p: any) => p.product_id)
                ...
                <div className="border rounded bg-white overflow-y-auto flex-grow mr-2 h-32">
                  <div className="flex flex-col p-2">
                    {promotion.embedded_discount_plans.length > 0 ? (
                      promotion.embedded_discount_plans.map((plan: any, index: number) => (
                        <div
                          key={index}
                          className="flex items-center mb-1 mr-1 border border-gray-300 rounded p-1"
                        >
