...
export default function InvoicePage() {
  ...
  const handleDownloadPDF = async () => {
    ...
    setLoading(true);
    try {
      const response: any = await helperService.post(ApiURL.v1Html2pdfGeneratePdfService, {
        content: htmlContent,
        filename: 'policies/' +(type == "partner" ? billing.data[0].billings.billing_no : billing.data[0].items[0].billings.billing_no)
      });
